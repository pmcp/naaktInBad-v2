<template>
  <div>
    ...
    ><span class="relative z-10  top-2 z-10 text-base font-body italic">{{ article.song }}</span></div>
    <vue-plyr
      v-if="article"
      ref="plyr"
      :options="options">
  ...
  data() {
    return {
      controls: `<div class="plyr__controls flex ">
          <button type="button" class="plyr__control mr-2 mb-6 rounded-none" aria-label="Play, {title}" data-plyr="play">
              <div class="relative icon--not-pressed w-4 h-4">
                <svg role="presentation" class="absolute hover:hidden " width="122" height="132" viewBox="0 0 122 132" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M0 0L121.919 65.933L0 131.879V0Z" fill="black"/></svg>
                <svg role="presentation" class="absolute hidden hover:block" width="136" height="146" viewBox="0 0 136 146" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5 7L126.919 72.933L5 138.879V7Z" stroke="black" stroke-width="8.0121" stroke-miterlimit="10"/></svg>
              </div>
              <div class="relative icon--pressed w-4 h-4 pr-1">
